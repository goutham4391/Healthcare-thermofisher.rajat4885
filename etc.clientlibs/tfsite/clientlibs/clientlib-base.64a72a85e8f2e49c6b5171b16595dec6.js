...
(function() {
    ...
    function Accordion(config) {
        ...
        function init(config) {
            ...
            if (that._elements["item"]) {
                ...
                var expandedItems = getExpandedItems();
                if($(".cmp-faqlist").attr('data-iswcmmode') == 'true'){
                    if (expandedItems.length === 0) {
                        toggle(0);
                    }
                }
        ...
        function expandItem(item) {
            ...
            if (index > -1) {
                ...
                //console.log("What is the item id here - " + index + " and " + item.id );
                var oneId = item.id;
                if (oneId.startsWith("faqlist")) {
                	document.dispatchEvent(new CustomEvent("faq:viewed", {}));
                	console.log("Add new event for faq - faq:viewed.  " + oneId );
                }
            }
        }
    ...
    function onDocumentReady() {
        ...
        // <IE11 does not have MutationObserver so don't throw errors for it
        if(MutationObserver){
...
(function() {
    ...
    //if lazy load via script needed to be implemented
    var isLazyViaScript = false;
    ...
    function Image(config) {
        ...
        function init(config) {
	        ...
	        if (that._properties.lazy && isLazyViaScript) {
                addLazyLoader();
            }
        ...
        function loadImage() {
            ...
            if (that._lazyLoaderShowing && isLazyViaScript) {
        ...
        function cacheElements(wrapper) {
            ...
            for (var i = 0; i < hooks.length; i++) {
                ...
                var key; 
                capitalized = capitalized.charAt(0).toUpperCase() + capitalized.slice(1);
                // >IE11 does not have dataset and this breaks other JS
                if(hook.dataset) {
                    key = hook.dataset[NS + "Hook" + capitalized];
                }
                that._elements[key] = hook;
            }
        }
...
(function ($) {
    $(document).ready(function () {
        /**
         * This function is used to create the section anchor-list
         */
        function createAnchorList() {
            $(".cmp-anchorlist__list").find("a").remove();

            var sectionId = "";
            var sectionText = "";
            var anchorLink = "";

            $.each($("[data-anchor-nav-title]"), function (index, value) {
                sectionText = $(value).attr("data-anchor-nav-title");
                sectionId = $(value).attr("id");
                anchorLink = '<div class="cmp-anchoritem"><a class="cmp-anchoritem__anchor" href="#' + sectionId + '"><span class="cmp-anchoritem__text">' + sectionText + '</span></a></div>';
                $(".cmp-anchorlist__list").append(anchorLink);
            });
        }
...
var productListCartHandler = (function ($this) {
	...
	$this.getFormValues = function (cartForm, additionalOmniParameters) {
		...
		if (cartForm != undefined && cartForm != "") {
			form = cartForm;
		}
			...
			if (hasQuantity == true) {
				...
				if (i % 10 == 9 && i != 1) {
					if (!isValidQuantity(quantity) && quantity != "") {
						...
						if (quantity != "") {
							var LineItem = productListCartHandler.addLineItem(sku, quantity, description, price, size, assayID, assayName, purity, scale, "", "", "", "", "", "");
		...
		$('#' + form + ' .ddsubscription , #' + form + ' .cbsubscription').each(function (index, element) {
			...
			if ($(this).hasClass("cbsubscription") && $(this).is(':checked') && nameExists(this, "addOnProductGroup")) {
				...
				var LineItem = productListCartHandler.addLineItem(sku, quantity, description, price, size, assayID, assayName, purity, scale, productRatePlanId, productRatePlanName, productRateChargeId, baseLineKey, baseLineSku, billingPeriod, lineNo, parentLineNo);
				lineItems.push(LineItem);
				...
				var LineItem = productListCartHandler.addLineItem(sku, quantity, description, price, size, assayID, assayName, purity, scale, productRatePlanId, productRatePlanName, productRateChargeId, baseLineKey, baseLineSku, billingPeriod, lineNo);
				lineItems.push(LineItem);
			}
...
(function() {
	var dropdownClassName = "cmp-dropdown";
	var openDropdownClassName = "open";
	var dropdownSelector = "." + dropdownClassName;
	var dropdownToggleSelector = ".cmp-dropdown__toggle";
	var dropdownMenuSelector = ".cmp-menu";
	var dropdownEls = document.querySelectorAll(dropdownSelector);
	...
	function initEvents(dropdownEl) {
		var toggleEl = dropdownEl.querySelector(dropdownToggleSelector);
	...
	function handleDocumentClick(event) {
		var toggleEl = event.target.closest(dropdownToggleSelector);
		var menuEl = event.target.closest(dropdownMenuSelector);
		
		// if we didn't click in a dropdown, close all dropdowns
		if(!menuEl && !toggleEl){
			dropdownEls.forEach(function(el){
				var currentToggleEl = el.querySelector(dropdownToggleSelector);

				el.classList.remove(openDropdownClassName)
				updateAriaValue(currentToggleEl, "false");
			});
		}
	}
	...
	function toggle(event){
		...
		var dropdownEl = toggleEl.parentNode;

		if(dropdownEl && dropdownEl.classList.contains(dropdownClassName)) {
			dropdownEl.classList.toggle(openDropdownClassName);
			updateAriaValue(toggleEl);
		}
	}

	function updateAriaValue(toggleEl, newValue) {
		var ariaExpandedValue = newValue || toggleEl.getAttribute("aria-expanded") === "true" ? "false" : "true";
		toggleEl.setAttribute("aria-expanded", ariaExpandedValue);
	}

	function init() {
		dropdownEls.forEach(initDropdown);
	}
...
(function ($) {
    ...
    // css class name for the link active state
    var ACTIVE_CSS_CLASS = 'cmp-dynamiccontentloader__link--active';
    var PARENT_ACTIVE_CSS_CLASS = 'cmp-dynamiccontentloader__link-parent--active';

    // only load the content within the container with the preset Id
    var LOAD_CONTENT_CLASS = '#main-content-chromeless';
    var DATA_ID_ATTRIBUTE = 'data-dynamicContent-id';

    var dynamicContentContainer;
    var linkList = [];
    var contentContainer;
    var beforeContentLoadedHandlers = {};
    ...
    function onDocumentReady() {
        var dataId;
        var linkContainerIds;

        dynamicContentContainer = document.querySelector('['+ DATA_ID_ATTRIBUTE +']');

        if (dynamicContentContainer) {
            dataId = dynamicContentContainer.getAttribute(DATA_ID_ATTRIBUTE);
            linkContainerIds = dynamicContentContainer.getAttribute('data-navContainerIds');

            contentContainer = document.getElementById(dataId);
            getContainerLinks(linkContainerIds);

            $(contentContainer).on("DCL.beforeContentLoaded", function(event, data) {
                if(data && data.href && data.beforeContentLoadedHandler) {
                    beforeContentLoadedHandlers[data.href] = data.beforeContentLoadedHandler;
                }
            });
        }
    ...
    /**
     * get anchor tags ('a') in containers
     */
    function getContainerLinks(linkContainerIds) {
        if (linkContainerIds) {
            linkContainerIds = linkContainerIds.split(',');
            linkContainerIds.forEach(function (item) {
                var container = document.getElementById(item);

                if (container) {
                    var links = container.querySelectorAll('a');
                    for (var i = 0; i < links.length; i++) {
                        addClickHandler(links[i]);
                        linkList.push(links[i]);
                    }
                } else {
                    console.log("DynamicContentLoader - Container not found: ", item);
                }
            })
        } else {
            console.log("DynamicContentLoader - No link Containers: ", linkContainerIds);
        }
    }
    ...
    function linkClickHandler(event) {
        ...
        if (event.currentTarget.href) {
            ...
            console.log('DynamicContentLoader - href not found: ', event.currentTarget);
        }
    ...
    /**
     * setSelected
     * loop over linklist and add css class to matching link
     * @param href
     */
    function setSelected(href) {
        ...
        linkList.forEach(function (link) {
            if (link.href === href) {
                link.parentElement.classList.add(PARENT_ACTIVE_CSS_CLASS);
                link.classList.add(ACTIVE_CSS_CLASS);
            } else {
                link.parentElement.classList.remove(PARENT_ACTIVE_CSS_CLASS);
                link.classList.remove(ACTIVE_CSS_CLASS);
            }
        })
    }
    ...
    function getPageContent(href) {
        // Consider showing a loading spinner

        $.ajax({
            type: 'GET',
            url: href
        }).done(function (response) {
            contentSuccessHandler(response, href)
        })
    }

    /**
     * find the content and display in the content container
     */
    function contentSuccessHandler(result, href) {

        var resultContainer = document.createElement("div");
        var beforeContentLoadedHandler = beforeContentLoadedHandlers[href];

        resultContainer.innerHTML = result;
        var chromelessContent = $(resultContainer).find(LOAD_CONTENT_CLASS).html();

        if (chromelessContent) {
            if(beforeContentLoadedHandler) {
                beforeContentLoadedHandler();
            }

            $(contentContainer).html(chromelessContent);

            $(contentContainer).trigger("DCL.contentUpdated");

        } else {
            $(contentContainer).html('Content not found!');
        }
    }
